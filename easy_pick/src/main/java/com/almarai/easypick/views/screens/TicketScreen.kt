...
class TicketScreen : Fragment() {
    ...
    private var files: List<Uri>? = null
    ...
    private fun init() {
        ...
        screenBinding.screenTicketSendButton.setOnClickListener {
            ...
            if (screenBinding.screenTicketDetailsEditText.text.toString().isNotBlank()) {
                viewModel.generateTicket(type, feedback, files)
                observeUploadResult()
            } else {
                showAlertDialog(alertMessage = R.string.alert_ticket_no_description)
            }
        }
    }
